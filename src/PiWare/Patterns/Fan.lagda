...
fan-⊕-M n = record { op = fan-⊕-M-op; op-<$> = fan-⊕-M-<$> }
  where
    ...
    fan-⊕-M-op (x ∷ xs) = uncurry′ _++_ ∘ map× id {!!} ∘ initLast′ $ (x ∷ xs)

    -- for free due to parametricity
    postulate fan-⊕-M-<$> : ∀ {α β} (f : α → β) (xs : Vec α (suc n)) → fan-⊕-M-op (map f xs) ≡ map f (fan-⊕-M-op xs)
\end{code}
